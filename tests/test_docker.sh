...
config_with_project='{
  "filesystem": {
    "allowWrite": ["/home/<user>/projects/myapp", "/tmp"],
    "denyRead": []
  }
}'
if output=$(echo "$config_with_project" | "$GRITGUARD_DIR/bin/generate-docker-args" 2>&1); then
    if echo "$output" | grep -q "\-\-workdir.*/home/<user>/projects/myapp"; then
...
if output=$("$GRITGUARD_DIR/bin/gritguard-docker" --repo "$TEST_REPO" touch /home/<user>/docker-evil.txt 2>&1); then
    log_fail "Write outside allowed paths should have failed"
    rm -f /home/<user>/docker-evil.txt 2>/dev/null
else
...
if output=$("$GRITGUARD_DIR/bin/gritguard-docker" --repo "$TEST_REPO" cat /home/<user>/.ssh/known_hosts 2>&1); then
...
if output=$("$GRITGUARD_DIR/bin/gritguard-docker" --repo "$TEST_REPO" ls /home/<user>/.gnupg 2>&1); then
...
if output=$("$GRITGUARD_DIR/bin/gritguard-docker" --repo "$TEST_REPO" ls /home/<user>/.aws 2>&1); then
    if [ -z "$output" ] || echo "$output" | grep -qE "(No such file|cannot access)"; then
